...
def test_narrowing_grades_from_the_receipt_with_the_store_deleted(store, tmp_path, audit_root):
    ...
    ``allowed_files`` narrowing are both proven from those receipts alone (#5418
    added glob-subsumption grading for the file axis).

    The root hop (receipts[0]) is structural-only and grades PASS (REASON_ROOT_STRUCTURAL_ONLY
    is in PASS_REASONS).  The non-root hop (receipts[1]) also passes because
    ``src/core`` is subsumed by ``src/**``.  Both hops PASS => chain PASS.
    """
    parent = _mint_orchestrator(store)
    ...
    assert rows[1].axes == ()  # allowed_files graded and held; no unsupported axis
    assert REASON_COMPARISON_AXIS_UNSUPPORTED not in rows[1].reasons
...
def test_cli_verify_exits_zero_and_prints_the_hop_count(store, audit_root, monkeypatch):
    ...
    # A recorded ``allowed_files`` on a root hop now grades PASS (pinned below).
...
def test_a_root_hop_with_file_scope_is_structural_only_and_cli_exits_zero(store, audit_root, monkeypatch):
    """A root hop that records ``allowed_files`` is structural-only and grades PASS (#5418).

    After #5418 the ``allowed_files`` axis is graded via glob-subsumption, so it
    is no longer ``comparison_axis_unsupported``.  A root hop carries
    REASON_ROOT_STRUCTURAL_ONLY, which is in PASS_REASONS (not UNPROVEN_REASONS),
    so the chain grades PASS and the CLI exits 0.
    """
    monkeypatch.setattr(delegation, "_audit_key", lambda: KEY)
    parent = _mint_orchestrator(store)
    ...
    assert verdict.unproven_hops == 0
    assert verdict.hops[0].axes == ()  # allowed_files IS in SCOPE_BODY_KEYS; root reports no unknown axes
    assert REASON_COMPARISON_AXIS_UNSUPPORTED not in verdict.hops[0].reasons
